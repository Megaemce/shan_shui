...
export function stroke(
    ...
    for (let i = 1; i < ptlist.length - 1; i++) {
        ...
        w = w * (1 - noise) + w * noise * Noise.noise(prng, i * 0.5, n0);
        ...
        vtxlist0.push(
            new Point(
                ptlist[i].x + w * Math.cos(a),
                ptlist[i].y + w * Math.sin(a)
            )
        );
        vtxlist1.push(
            new Point(
    ...
    vtxlist = [ptlist[0]]
        .concat(
            vtxlist0.concat(
                vtxlist1.concat([ptlist[ptlist.length - 1]]).reverse()
            )
        )
        .concat([ptlist[0]]);

    return poly(vtxlist, 0, 0, fill, strokeColor, out);
}
...
export function blob(
    ...
    const plist = blob_points(prng, x, y, ang, len, strokeWidth, noise, fun);
...
export function blob_points(
    ...
    for (let i = 0; i < lalist.length; i++) {
        const ns = nslist[i] * noise + (1 - noise);
...
export function texture(
    ...
    for (let i = 0; i < tex; i++) {
        ...
        for (let j = start; j < end; j++) {
            ...
            const nx = noise(layer + 1) * (Noise.noise(prng, x, j * 0.5) - 0.5);
            const ny = noise(layer + 1) * (Noise.noise(prng, y, j * 0.5) - 0.5);
